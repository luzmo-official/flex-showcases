...
export class ScrollService implements OnDestroy {
  ...
  supportManualScrollRestoration: boolean = !!this.window && ('scrollTo' in this.window)
      && ('pageXOffset' in this.window) && isScrollRestorationWritable();
  ...
  get topOfPageElement() {
    if (!this._topOfPageElement || this._topOfPageElement === this.document?.body) {
      this._topOfPageElement = this.document?.querySelector('#top-of-page') || this.document?.body;
    }
  ...
  get scrollContainerElement() {
    if (!this._scrollContainerElement || this._scrollContainerElement === this.document?.body) {
      this._scrollContainerElement = this.document?.querySelector('mat-sidenav-content') || this.document?.body;
    }
  ...
  constructor() {
    ...
    if (this.window?.location?.href !== this.getStoredScrollLocationHref()) {
      this.removeStoredScrollInfo();
    }
  }
  ...
  scroll() {
    ...
    const element: any = hash ? this.document?.querySelector(`#${hash}`) ?? null : this.topOfPageElement;
  ...
  scrollToElement(element: HTMLElement|null) {
    if (element && element.scrollIntoView) {
      ...
      if (this.window?.scrollBy) {
        ...
        this.window.scrollBy({ left: 0, top: element.getBoundingClientRect().top - this.topOffset, behavior: 'instant' });

        // If we are very close to the top (<20px), then scroll all the way up.
        // (This can happen if `element` is at the top of the page, but has a small top-margin.)
        if (this.window.scrollY < 20) {
          this.window.scrollBy(0, -this.window.scrollY);
        }
      }

      this.scrollContainerElement.scrollTop = Math.max(0, this.scrollContainerElement.scrollTop - 16);

      if (this.scrollContainerElement.scrollTop < 32) {
  ...
  updateScrollLocationHref(): void {
    this.storage?.setItem('scrollLocationHref', this.window?.location?.href);
  }
  ...
  private getCurrentHash() {
    return decodeURIComponent(this.platformLocation.hash.replace(/^#/, '').replaceAll('/', ''));
  }
}
